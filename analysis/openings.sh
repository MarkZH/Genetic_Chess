...
grep -A$((moves-1)) '^1\.' "$game_file" | ./"$bindir"/delete_comments.sh | grep -v -- -- | paste -d' ' $paste_dashes | tee "${1}_opening_list.txt" | sort | uniq -c | sort -n | tee >(wc -l; echo different openings)
