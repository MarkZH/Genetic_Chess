...
xl = xlim;
yl = ylim;
avg = mean(moves_in_game);
stddev = std(moves_in_game);
md = mode(moves_in_game);
med = median(moves_in_game);
stats = {['Mean = ' num2str(avg)], ...
         ['Median = ' num2str(med)], ...
         ['Mode = ' num2str(md)], ...
         ['\sigma = ' num2str(stddev)]};
text(0.7*xl(2), 0.9*yl(2), stats, 'fontsize', 14);
