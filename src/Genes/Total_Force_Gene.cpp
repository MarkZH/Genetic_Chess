...
double Total_Force_Gene::score_board(const Board& board, Piece_Color perspective, size_t, double game_progress) const noexcept
{
    return std::accumulate(Square::all_squares().begin(), Square::all_squares().end(), 0.0,
                           [this, &board, perspective, game_progress](auto sum, auto square)
                           {
                               auto piece = board.piece_on_square(square);
                               if(piece && piece.color() == perspective)
                               {
                                   return sum + piece_strength_source->piece_value(piece, game_progress);
                               }
                               else
                               {
                                   return sum;
