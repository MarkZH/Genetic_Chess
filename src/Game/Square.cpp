...
Square::Square(char file, int rank) noexcept : square_index(8*(file - 'a') + (rank - 1))
{
    assert(inside_board());
}
...
bool Square::inside_board(char file) noexcept
{
    return 'a' <= file && file <= 'h';
}

bool Square::inside_board(int rank) noexcept
{
    return 1 <= rank && rank <= 8;
}

bool Square::inside_board(char file, int rank) noexcept
{
...
Squares_in_a_Line Square::squares_between(Square a, Square b) noexcept
{
    return Squares_in_a_Line(a, b);
}

Squares_in_a_Line Square::square_line_from(Square origin, Square_Difference direction) noexcept
{
...
Square_Line_Iterator::Square_Line_Iterator(Square start, const Square_Difference& square_step) noexcept :
...
Squares_in_a_Line::Squares_in_a_Line(Square first_border, Square second_border) noexcept :
...
Squares_in_a_Line::Squares_in_a_Line(Square origin, const Square_Difference& square_step) noexcept :
    border1(origin),
    border2{},
    step(square_step)
{
...
All_Squares_Iterator::All_Squares_Iterator(Square start) noexcept : current_square(start)
{
}
