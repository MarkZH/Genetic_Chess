...
Board::Board(const std::string& original_fen) : board_type(String::contains(original_fen, '*') ? Board_Type::MUSKETEER : Board_Type::STANDARD)
{
    ...
    if(String::contains(original_fen, '*'))
    {
        auto tokens = String::split(original_fen);
        auto board_rows = String::split(tokens.front(), "/");
        fen_parse_assert(board_rows.size() == 10, original_fen, "Wrong number of rows for Musketeer chess board.");
        for(auto gate_index : {0, 9})
        {
            auto gate = board_rows[gate_index];
            auto gate_color = gate_index == 0 ? Piece_Color::BLACK : Piece_Color::WHITE;
            fen_parse_assert(gate.size() == 8, original_fen, "Wrong number of characters in gated row for " + color_text(gate_color) + ".");
        ...
        fen_parse_assert(gated_piece_types.size() == 2, original_fen, "Could not identify the two gated pieces.");
        set_unmoved_gate_guardians();
    }

    auto starting_fen = String::remove_extra_whitespace(original_fen);
    fen_parse_assert(fen() == starting_fen, starting_fen, "Result: " + fen());
...
{
    starting_hash = board_hash();
    starting_fen_from_starting_hash[starting_hash] = fen;
}
