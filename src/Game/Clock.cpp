...
Game_Result Clock::punch(const Board& board) noexcept
{
    ...
    const auto player_index = static_cast<size_t>(whose_turn);
...
{
    moves_since_clock_reset[static_cast<size_t>(whose_turn)] -= 1;
    moves_since_clock_reset[static_cast<size_t>(opposite(whose_turn))] -= 1;
    punch({});
}
...
void Clock::stop() noexcept
{
    ...
    timers[static_cast<size_t>(whose_turn)] -= (time_stop - time_previous_punch);
...
Clock::seconds Clock::time_left(const Piece_Color color) const noexcept
{
    ...
    {
        return timers[static_cast<size_t>(color)];
    }
    else
    {
        const auto now = std::chrono::steady_clock::now();
        return timers[static_cast<size_t>(color)] - (now - time_previous_punch);
    }
}
...
size_t Clock::moves_until_reset(const Piece_Color color) const noexcept
{
    ...
    {
        return move_count_reset - moves_since_clock_reset[static_cast<size_t>(color)];
    }
    else
    {
...
{
    timers[static_cast<size_t>(player)] = new_time_seconds;
    time_previous_punch = std::chrono::steady_clock::now();
}

void Clock::set_increment(const Piece_Color player, const seconds new_increment_time_seconds) noexcept
{
    increment_time[static_cast<size_t>(player)] = new_increment_time_seconds;
}
...
void Clock::set_next_time_reset(const Piece_Color player, const size_t moves_to_reset) noexcept
{
    ...
    {
        move_count_reset = moves_since_clock_reset[static_cast<size_t>(player)] + moves_to_reset;
    }
}
...
{
    return increment_time[static_cast<size_t>(color)];
}
