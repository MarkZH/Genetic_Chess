...
To speed up evolution, the size of a gene's priority mutation is now proportional to the square root of the current value of the priority (with a minimum mutation size of 10). This allows for more rapid adjustment of values that are deemed important early on. This is a compromise between a constant mutation rate (which leads to a maximum linear growth) and a linearly proportional mutation rate (which leads to overly fast exponential growth).
