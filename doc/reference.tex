...
The \code{Complete\_Move}s returned by the methods \code{Board::get\_complete\_move()} and \code{Board::legal\_moves()} are the only moves guaranteed to be valid for a given \code{Board} instance, and only for the unmodified originating board or an unmodified copy. Calling \code{Board::submit\_move()} with a \code{Complete\_Move} from a different source will have three possible results:
\begin{enumerate}
...
\end{enumerate}
The legality of moves are only checked by the \code{Board} methods that generate legal \code{Complete\_Move} instances (the previously mentioned \code{Board::get\_complete\_move()} and \code{Board::legal\_moves()}), not on submission.
