...
\end{enumerate}
When the AI starts the algorithm to choose a move, it allocates a fraction of the time left on the clock so that all subsequent moves get equal time. That is, if there are 20~moves estimated to be left in the game, then \(1/20\)~of the remaining time is used for this move. The number of moves left is estimated by assuming the number of moves in a game is modeled by a log-normal distribution~\cite{log-norm-wiki}\cite{log-norm-chess-se} with a mean and spread given by the first two parameters listed above. Originally, it was assumed that the number of moves in a game was well-modeled by a Poisson distribution. This proved to be false (see Figure~\ref{game-length-plot}). The number of moves left in the game is estimated by
\[N = \frac{\sum_{i = n + 1}^\infty P(i)\times{}i}{\sum_{i = n + 1}^\infty P(i)} - n\]
where \(N\) is the estimated total number of moves in the game, given that \(n\) moves have already ben made, and \(P(i)\) is the \emph{a priori} probability distribution. The log-normal probability distribution is given by
\[P(i) = \frac{1}{iS\sqrt{2\pi}} e^{-\frac{1}{2}\left(\frac{\ln{i} - M}{S}\right)},\]
where \(S\) and \(M\) are the standard deviation and mean of \(\ln{N}\), respectively. A plot of the number of moves in a long gene pool run can be seen in Figure~\ref{log-norm-plot}. Instead of using the summation formula above to estimate the number of moves remaining in the game, the result can be approximated by an integral to yield a closed-form expression using function available in native \cpp.
\[
\sum_{i = n + 1}^\infty P(i)\times{}i \approx \int_{n+1}^\infty P(t)tdt = \frac{1}{S\sqrt{2\pi}} \int_{n+1}^\infty e^{-\frac{1}{2}\left(\frac{\ln{t} - M}{S}\right)}dt
\]
\[
...
and
\[
\sum_{i = n + 1}^\infty P(i) \approx \int_{n+1}^\infty P(t)dt = \frac{1}{S\sqrt{2\pi}} \int_{n+1}^\infty \frac{1}{t} e^{-\frac{1}{2}\left(\frac{\ln{t} - M}{S}\right)}dt
\]
\[
= \frac{1}{2}\left[1 + \textrm{erf}\left(\frac{M-ln_x}{Sr2}\right)\right].
\]
Here, \(\textrm{erf}\) is the error function given by
\[
\textrm{erf}(x) = \frac{2}{\sqrt{\pi}}\int_0^x e^{-t^2}dt
\]
This results in the number of moves left in a game after \(n\) moves as
\[
N_{\textrm{left}}(n) = \left(e^{M + S^2/2}\right) \frac
{1 + \textrm{erf}\left(\frac{M + S^2 - \ln n}{S\sqrt{2}}\right)}
{1 + \textrm{erf}\left(\frac{M-ln_x}{Sr2}\right)}
- n.
\]
\begin{figure}[tbh]
	\centering
