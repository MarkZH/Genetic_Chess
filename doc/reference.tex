...
Though the operators \code{new} and \code{delete} appear nowhere in the code and smart pointers are used, raw pointers appear everywhere in the code. There are two driving reasons for using pointers: polymorphic storage and preventing of unnecessary copying. As an example of the first reason, classes representing moves are derived from a \code{Move} class and need to be stored within each instance of the \code{Piece} classes. Storing different types of moves requires storing \code{Move*}s. The same goes for \code{Gene} storage in the \code{Genome} class. For the second reason, consider the \code{Piece} class. There is no difference between, say, any two white rooks, even if they exist on different \code{Board}s. So, it would be a waste of time and memory to have multiple copies of the white rook on the same board or different boards. So, all pointers to a white rook ultimately point to the same instance, which is stored as a static member of the \code{Board} class.
