...
\emph{Update:} The idea in the above paragraph has been revisited in a different form. See Section~\ref{priority-threshold}.
...
\begin{enumerate}
	\item a smooth function of the parameters of its genome and
...
This gene stops any other board-scoring gene from running its \code{score\_board()} if that other gene's priority is too low, thus trading lower score detail for faster evaluations. It contains a genetically determined threshold value that is referenced during the genome's \code{evaluate\_board()} method. Instead of using the value directly, the actually threshold used is a random number uniformly distributed between 0.0 and the threshold value.  The reason for this is to prevent low-priority genes from being permanently cut off from evolutionary pressure while still providing a means of speeding up evaluations.

Since skipping \code{score\_board()} methods will tend to lower the score given to a position (assuming all genes have positive priorities), the resulting score is scaled by the ratio of the sum of all genes' priorities to the sum of all used genes' priorities. This way, board positions with higher scores due to using more genes to score it will not be unduly favored over board positions with a lower score due to using fewer genes. To put it in mathematical terms:
\[Final\ score = Score\times\frac{Sum\ of\ all\ priorities}{Sum\ of\ all\ used\ priorities}\]
...
The gene counts the number of pawns that cannot be stopped or captured by an opponent's pawns. That is, a pawn is a passed pawn if there are no pawns ahead of it in its file or any directly adjacent files.
