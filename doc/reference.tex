...
The time allocation adjustment rate for the speculation constants either increases or decreases the effective constant by the following formula:
\[C' = C(1+r)^d\]
where \(C'\) is the new speculation constant, \(C\) is the default value of that constant, \(r\) is the adjustment rate, and \(d\) is the current depth of the search. If the adjustment rate is positive, then more time will be allocated at greater depths. If negative, increasingly less time will be allocated at greater depths.
