...
\begin{itemize}
\item The \code{Complete\_Move}s returned by the methods \code{Board::get\_complete\_move()} and \code{Board::legal\_moves()} are only valid for the unmodified originating board. In this, they are similar to iterators from STL containers. There is no guarantee that a \code{Complete\_Move} is valid if the board changes state after getting a copy of the move.
