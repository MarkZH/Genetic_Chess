...
\begin{description}
	...
	\item[\code{genetic\_chess -confirm <file name>}] Analyze a game recod in Portable Game Notation (PGN \cite{pgn-file-format}) to check that all moves listed are legal and all move are correctly noted with respect to check (+), capture (x), and checkmate (\#).
...
Internally, the values of the piece strengths are normalized so that the sum of the absolute values of all the pieces of one color is one so that the priorities of the Total Force Gene and the Opponent Pieces Targeted Gene can mutate independently.
